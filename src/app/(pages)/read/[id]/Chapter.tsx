...
const Chapter = ({ dataRead, id }: { dataRead: chapterType; id: string }) => {
  ...
  const handleNext = useCallback(() => {
    if (dataRead?.has_next?.is_next_link === true) {
      router.push(`/read${dataRead?.has_next?.has_next_link}`);
    }
  }, [
    dataRead?.has_next?.is_next_link,
    dataRead?.has_next?.has_next_link,
    router,
  ]);
  const handlePrevious = useCallback(() => {
    if (dataRead?.has_prev?.is_prev_link === true) {
      router.push(`/read${dataRead?.has_prev?.has_prev_link}`);
    }
  }, [
    dataRead?.has_prev?.is_prev_link,
    ...
    <>
      ...
      <div className="w-[95%] sm:w-[90%] m-auto ">
        <div className="title mt-10">
          <h1 className="font-noto-sans text-xl sm:text-2xl text-center text-white">
            Komik {dataRead?.title}
          </h1>
          <hr className="mt-6" />
          <p className="text-center text-sm mt-2 text-white font-karla font-medium">
            Kamu sedang berada di halaman baca komik{" "}
            <strong>{dataRead?.title}</strong>
            bahasa Indonesia. Jika kamu ingin membaca manga{" "}
            <strong>{dataRead?.title}</strong>, pastikan Javascript kalian aktif
            . Untuk koleksi komik seru lainnya di <strong>Mangazuna</strong> ada
            di menu Daftar Komik.
          </p>
        </div>
        <div className="flex justify-center mt-4">
          <div className="hasEpisode flex gap-2">
            {dataRead?.has_prev?.is_prev_link === true ? (
              <button
      ...
      <div className="mb-20 w-full sm:w-[60%] m-auto">
        ...
        <div className="justify-end mt-6 flex gap-3">
          ...
          {dataRead?.has_next?.is_next_link === true ? (
            <button
              className="rounded-md px-5 py-2 text-sm bg-[#282a33] hover:bg-[#23242c] text-white"
              onClick={handleNext}
            >
