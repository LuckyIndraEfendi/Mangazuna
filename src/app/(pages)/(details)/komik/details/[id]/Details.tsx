...
const Details = async ({
  ...
  const images = dataDetails?.images
    ? dataDetails?.images.replace(/\?.*$/, "")
    : "";
  const title = id
    ? id
        .replace(/-/g, " ")
      ...
      <div className="">
        ...
        <div className="w-[95%] m-auto relative -top-32 ">
              ...
              <div className="py-2  flex flex-col sm:col-span-7">
                ...
                <ul className="flex gap-3 mt-3 justify-center sm:justify-start flex-wrap">
                  <li className="bg-orange-400 text-white font-karla px-2 py-[2px] rounded-md font-semibold">
                    {dataDetails?.chapter_list?.chapter_list.length} Chapter
                  </li>
                  <li className="bg-orange-400 text-white font-karla px-2 py-[2px] rounded-md font-semibold">
                    {dataDetails?.rating}
                  </li>
                  <li className="bg-orange-400 text-white font-karla px-2 py-[2px] rounded-md font-semibold">
                    {dataDetails?.jenis_komik}
                  </li>
                  <li className="bg-orange-400 text-white font-karla px-2 py-[2px] rounded-md font-semibold">
                    {dataDetails?.konsep_cerita}
                  </li>
                  <li className="bg-orange-400 text-white font-karla px-2 py-[2px] rounded-md font-semibold">
                    {dataDetails?.type}
                  </li>
                  <li className="bg-orange-400 text-white font-karla px-2 py-[2px] rounded-md font-semibold">
                    {dataDetails?.pengarang}
                  </li>
                  <li className="bg-orange-400 text-white font-karla px-2 py-[2px] rounded-md font-semibold">
                    {dataDetails?.dilihat}
                  </li>
                  <li className="bg-orange-400 text-white font-karla px-2 py-[2px] rounded-md font-semibold">
                    {dataDetails?.update_terakhir}
                  </li>
                </ul>
                <Synopsis synopsis={dataDetails?.synopsis} />
              </div>
            </div>
            <div className="mt-4 flex gap-5 items-center flex-col sm:flex-row ">
              <NavLink
                href={`/komik/read${
                  dataDetails?.chapter_list?.chapter_list[
                    dataDetails?.chapter_list?.chapter_list.length - 1
                  ]?.chapter_link
                }`}
              >
              ...
              <div className="moreIcon flex gap-2  ">
                <Bookmark
                  ...
                  title={title}
                  totalChapter={dataDetails?.chapter_list?.chapter_list.length}
                  session={session}
                />
          ...
          <div id="episodeList" className="mt-6">
            <ul
              className={`${
                dataDetails?.chapter_list?.chapter_list?.length
                  ? "h-[50vh]"
                  : "h-[80vh]"
              } overflow-y-scroll `}
              id="customScrollThumb"
            >
              {dataDetails?.chapter_list?.chapter_list.map(
                (item: any, i: number) => (
                  <NavLink href={`/komik/read${item.chapter_link}`} key={i}>
                    <li className="flex justify-between px-5 py-3 font-karla text-lg bg-[#141519] text-white cursor-pointer hover:bg-[#17191d] visited:text-yellow-700">
                      <div className="flex gap-4">
                        ...
                        <span>Episode {item.chapter_update}</span>
                      </div>
          ...
          <div id="recomendation" className="sm:mt-5 mt-10">
            ...
            <div className="mt-6">
              <CarouselDetails
                komikLainnya={dataDetails?.komik_lainnya?.komikLainnya}
                id={id}
              />
            </div>
          </div>
